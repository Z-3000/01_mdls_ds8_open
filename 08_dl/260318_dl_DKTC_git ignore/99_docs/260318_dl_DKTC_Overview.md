...
DKTC 훈련 데이터를 이용해 협박, 갈취, 직장 내 괴롭힘, 기타 괴롭힘, 일반 대화 5가지 대화 유형 Class를 분류하는 딥러닝 모델을 만듭니다. DKTC 테스트 데이터를 활용해 분류 성능을 평가합니다. 리더보드를 이용해봅니다.

원본 과제 참고 링크 [Tunib GitHub](https://github.com/tunib-ai/DKTC)

**Evaluation**

모델이 분류한 결과와 정답 간의 f1 score로 측정합니다.

Submission.csv 테스트 세트의 각 ID에 대해 TARGET 변수의 확률을 예측해야 합니다. 제출 파일은 다음 형식의 헤더를 포함해야 합니다.
